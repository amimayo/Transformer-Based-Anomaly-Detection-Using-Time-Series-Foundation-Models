...
def evaluate(anomaly_scores, labels, algo, window_size):
    ...
    print("\n" + "=" * 45)
    print(f" {algo.upper()} CROSS-MACHINE SCORECARD")
    print("=" * 45)
    for metric, value in results_dict.items():
        print(f"{metric.ljust(25)} : {value:.3f}")


def run_cross_machine_test(train_path, test_path, test_label_path, algo, window_size=100):

    print("Transformer-Based Anomaly Detection Using Time Series Foundation Models Baseline Cross Machine Test")

    print(f"FEW SHOT RATE : 0.7")
...
def main():
    ...
    train_path = "./ServerMachineDataset/train/machine-1-7.txt"
    test_path = "./ServerMachineDataset/test/machine-1-7.txt"
    test_label_path = "./ServerMachineDataset/test_label/machine-1-7.txt"

    run_cross_machine_test(train_path, test_path, test_label_path, algo, window_size)
